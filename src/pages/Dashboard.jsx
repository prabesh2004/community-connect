...
export default function Dashboard() {
    return (<div className="container mx-auto py-16 px-4 text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => (<motion.div key={stat.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: index * 0.08 }}>
      ...
      <div className="mt-10 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-white/5 border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.7)]">
          ...
          <CardContent>
            <div className="space-y-4">
              {['Environment', 'Education', 'Hunger Relief', 'Community Care'].map(cause => (<div key={cause} className="flex items-center justify-between text-sm text-white/60">
                  <span>{cause}</span>
                  <span>Active</span>
                </div>))}
            </div>
