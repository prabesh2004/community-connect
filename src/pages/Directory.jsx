...
export default function Directory() {
    ...
    return (<div className="container mx-auto py-12 text-white">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8 items-end">
        ...
        <div className="w-full md:w-1/2 flex flex-wrap gap-2">
          {causes.map(cause => (<Badge key={cause} variant="outline" className={`cursor-pointer text-xs py-1 px-3 border transition-colors ${selectedCause === cause
                ? 'bg-white text-black border-white'
                : 'bg-black text-white/70 border-white/20 hover:border-white'}`} onClick={() => setSelectedCause(cause)}>
              {cause}
            </Badge>))}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-10">
        {skills.map(skill => (<Badge key={skill} variant="outline" className={`cursor-pointer text-xs py-1 px-3 border transition-colors ${selectedSkill === skill
                ? 'bg-white text-black border-white'
                : 'bg-black text-white/70 border-white/20 hover:border-white'}`} onClick={() => setSelectedSkill(skill)}>
            {skill}
          </Badge>))}
      </div>

      {filteredNonprofits.length === 0 ? (<div className="text-center py-20 text-muted-foreground">
          ...
          {filteredNonprofits.map(np => (<Card key={np.id} className="flex flex-col overflow-hidden border border-white/10 bg-white/5 hover:border-white transition-all shadow-[0_20px_60px_rgba(0,0,0,0.7)]">
              <div className="h-48 w-full bg-cover bg-center" style={{ backgroundImage: `url(${np.image})` }}/>
              <CardHeader>
              ...
              <CardContent className="flex-grow">
                ...
                <div className="flex flex-wrap gap-1">
                  {np.needs.map(need => (<Badge key={need} variant="outline" className="text-xs border-white/20 text-white/70">
                      {need}
                    </Badge>))}
                </div>
              </CardContent>
